...
def GetAbortedClients_Num(user, passwd, port, database):
    ...
    aborted_clients_num_conn = get_connection(host, user, passwd, port, database)
    ...
    except aborted_clients_num_conn.Error as e:
        return None
    finally:
...
def GetFailConnect_Num(user, passwd, port, database):
    ...
    failure_connect_num_conn = get_connection(host, user, passwd, port, database)
    ...
    except failure_connect_num_conn.Error as e:
        return None
    finally:
...
def GetSlowQuery_Num(user, passwd, port, database):
    ...
    slow_query_num = ''
    select_slow_query_num_conn = get_connection(host, user, passwd, port, database)
    ...
    except select_slow_query_num_conn.Error as e:
        return None
    ...
    get_slow_query_num_cmd = "grep Query_time {} | wc -l".format(select_slow_query_log_file)
...
def main(pid=GetProcess_Pid(), host='127.0.0.1', user='Rootmaster', passwd='Rootmaster@777', port=18103, database=None,**kwargs):
    process_message = {}
    ...
    process_message["conn_num"] = GetMysql_ConnNum(user, passwd, port, database)
    process_message["aborted_clients"] = GetAbortedClients_Num(user, passwd, port, database)
    process_message["failure_connect"] = GetFailConnect_Num(user, passwd, port, database)
    process_message["slow_query"] = GetSlowQuery_Num(user, passwd, port, database)
    process_message['backup_status'] = GetMysql_Backup(user, passwd, port, database)
