...
用户可以在应用商店发布“基础组件”与“应用服务”两个维度的产品，在区分上，应用服务可以理解为完整的提供某一类服务的产品，产品内部可由一个或多个“服务”组成
，比如gitlab、jenkins等。基础组件的角色更多是作为其他完成产品的一部分存在，以完成产品的某些功能需求，如mysql、redis等。
...
以MySQL服务为例，需将涉及到的文件统一放在 mysql目录下，目录名称与该服务名称保持一致，目录中需要提供与该目录名称一致的配置文件(
如：mysql.yaml)、产品图标（如：mysql.svg)  和其他所需文件（如安装脚本等）
...
2. 确保目录名称(mysql)、配置文件(mysql.yaml) 、图标(mysql.svg) 名称统一,
   上传安装包时，平台将根据名称校验对应文件合法性，如名称不一致，可能会导致校验不通过等问题；
3. 确保安装包解压后是一个整体目录
...
monitor:
  # 以下字段用于服务监控，优先监控端口，其次监控进程，如果均未获取到则不监控该服务
...
control:
  ...
  # 停止脚本路径，如没有可留空 （类型：string）
  stop: "./scripts/mysql stop"
  # 重启脚本路径，如没有可留空 （类型：string）
  restart: "./scripts/mysql restart"
  # 重载脚本路径，如没有可留空 （类型：string）
  reload:
  # 安装脚本路径，必填 （类型：string）
  install: "./scripts/install.py"
  # 初始化脚本路径，必填 （类型：string）
  init: "./scripts/init.py"
```

### 2.4. 安装流程说明

在成功发布基础组件包 用户选择安装，可以填写配置文件(yaml)中 `install` 字段内指定的内容，`ports`
字段中指定内容，选择`dependencies`依赖组件相关信息, 然后执行 `control`中定义的`install`
脚本，平台会将以上数据组合成json内容传入到脚本中，脚本根据json中的值进行安装工作。
...
发布类别为应用服务的产品时，目录名称、文件名称需要做到如下统一：

1. 产品目录名需要与产品名称保持一致
2. 产品图标名需要与产品名称保持一致
3. 产品配置文件名需要与产品名称保持一致
4. 产品包含服务目录名需要与产品名称保持一致
5. 服务配置文件名需要与服务包名保持一致

例：

发布产品名称为“prod_a",其中包含 3个服务为“service_a","service_b","service_c" 的目录结构如下

```shell
$ tree prod_a
prod_a
├── prod_a.svg # 定义产品图标，会在平台中展示，如果不创建则平台会展示缺省图标
├── prod_a     # 定义产品下服务配置文件目录，将所需服务的配置文件存在该目录
│   ├── service_a.yaml  # 服务 service_a 配置文件，文件名需要与服务名称一致
│   ├── service_b.yaml  # 服务 service_b 配置文件，文件名需要与服务名称一致
│   └── service_c.yaml  # 服务 service_c 配置文件，文件名需要与服务名称一致
├── service_a-1.2.4-5d1ac8ce87323fc399506d1335ae5c98.tar.gz  # 服务 service_a 压缩包，以“-” 为分隔符，第一个为服务名称，需要与服务名称一致，格式为 {service_name}-{service_version}-{others}-{package_md5}.tar.gz
├── service_b-1.2.4-5d1ac8ce87323fc399506d1335ae5c98.tar.gz  # 服务 service_b 压缩包
├── service_c-1.2.4-5d1ac8ce87323fc399506d1335ae5c98.tar.gz  # 服务 service_c 压缩包
└── prod_a.yaml	# 定义产品配置文件，文件名需要与产品名称一致
...
```shell
$ tree service_a
service_a								# 目录名称，请与服务名称一致
├── service_a.yaml			# 服务配置文件, 记录该服务安装、升级等所需信息, 请使用 “服务名称.yaml” 命名，与目录名称保持一致
├── others...						# 该服务涉及服务自身目录、文件
└── scripts							# 服务的安装、启动等控制脚本，该脚本在安装时会调用
...
例如： prod_a-1.2.1-beta-5d1ac8ce87323fc399506d1335ae5c98.tar.gz

### 3.3. 配置文件yaml说明

发布类别为应用服务的产品时，需分别对 产品配置文件和服务配置文件进行配置，具体如下
...
install:
  ...
  - name: "安装目录"
    ...
    key: base_dir
    # 上面key默认值 （类型： string）
    default: "{data_path}/service_a"  # 注： {data_path} 为主机数据目录占位符，请勿使用其他代替
  - name: "JVM设置"
    key: jvm
    default: "-XX:MaxPermSize=512m -Djava.awt.headless=true"
# 程序控制脚本与服务目录的相对路径 （类型：map）
control:
  # 启动脚本路径(shell实现)，如没有可留空,脚本名称建议与服务名称一致  （类型：string）
  start: "./bin/service_a start"
  # 停止脚本路径(shell实现)，如没有可留空，脚本名称建议与服务名称一致 （类型：string）
  stop: "./bin/service_a stop"
  # 重启脚本路径(shell实现)，如没有可留空，脚本名称建议与服务名称一致 （类型：string）
  restart: "./bin/service_a restart"
  # 重载脚本路径(shell实现)，如没有可留空 （类型：string）
  reload:
  # 安装脚本路径(python实现)，必填 （类型：string）
  install: "./scripts/install.py"
  # 初始化脚本路径(python实现)，必填 （类型：string）
  init: "./scripts/init.py"
```
