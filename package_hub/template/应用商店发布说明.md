...
├── mysql_binary				# 组件包程序目录，该目录存放组件核心文件
...
monitor:
  # 监控进程名称，如“mysqld”，平台在发现mysqld进程不存在后，会发送告警提醒 （类型：string）
  process_name:  "mysqld"
  # 监控端口号，请根据 ports 中的变量设置 （类型： {string}）
...
在成功发布基础组件包 用户选择安装，可以填写配置文件(yaml)中 `install` 字段内指定的内容，`ports`字段中指定内容，选择`dependencies`依赖组件相关信息, 然后执行 `control`中定义的`install`脚本，平台会将以上数据组合成json内容传入到脚本中，脚本根据json中的值进行安装工作。
...
[
    {
        ...
        "version": "2.0.1",
        "ports": [
            {"key": "service_port", "name": "xxx端口", "default": 8001}
        ],
        "install_arg": [
            {"key": "base_dir", "name": "服务目录", "default": "/data/app/nacos"},
            {"key": "data_dir", "name": "数据目录", "default": "/data/appData/nacos"},
            {"key": "username", "name": "用户名", "default": "/data/appData/nacos"},
            {"key": "password", "name": "密码", "default": "/data/appData/nacos"},
        ],
        "deploy_mode": {},
        "cluster_name": None,
        "instance_name": "nacos-1",
        "dependence": [
            {"name": "mysql", "instance_name": "aaa", "cluster_name": "bbb"},
            {"name": "mysql", "connect": "aaa"},
        ],
    },
    {
        "name": "mysql"
...
monitor:
  # 监控进程名称，如“service_a”，平台在发现service_a进程不存在后，会发送告警提醒 （类型：string）
