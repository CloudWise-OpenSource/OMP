...
class ServiceGatewayServer(Prometheus):
    """
    查询 prometheus GatewayServer 指标
    """
    ...
    def service_status(self):
        ...
        expr = f"probe_success{{env=~'{self.env}'," \
               f"instance=~'{self.instance}',app=~'gatewayServer',app!='node'}}"
        self.ret['service_status'] = self.unified_job(*self.query(expr))

    def run_time(self):
        """运行时间"""
        expr = f"process_uptime_seconds{{env=~'{self.env}'," \
               f"instance=~'{self.instance}',job=~'gatewayServerExporter'}}"
    ...
    def cpu_usage(self):
        ...
        expr = f"system_cpu_usage{{env=~'{self.env}'," \
               ...
               f"job='gatewayServerExporter'}} * 100"
