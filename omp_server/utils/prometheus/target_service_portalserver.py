...
class ServicePortalServer(Prometheus):
    """
    查询 prometheus PortalServer 指标
    """
    ...
    def service_status(self):
        ...
        expr = f"probe_success{{env=~'{self.env}'," \
               f"instance=~'{self.instance}',app=~'portalServer'," \
               f"app!='node'}}"
    ...
    def run_time(self):
        ...
        expr = f"process_uptime_seconds{{env=~'{self.env}'," \
               f"instance=~'{self.instance}',job=~'portalServerExporter'}}"
    ...
    def cpu_usage(self):
        ...
        expr = f"system_cpu_usage{{env=~'{self.env}'," \
               ...
               f"job='portalServerExporter'}} * 100"
